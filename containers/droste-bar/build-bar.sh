#!/usr/bin/env bash
# Build the droste-bar OCI base image from a seed tarball.
#
...
#
# droste-bar is the shared core: everything in seed that works in
# plain OCI containers. droste-foo layers on top to add init/systemd
# back for LXC system container use.
#
# Usage:
#   build-bar.sh
#   build-bar.sh --seed /path/to/droste-seed.tar.xz
#   build-bar.sh --no-import    # Build rootfs only, skip container import
set -euo pipefail
...
# ── Work directory ──────────────────────────────────────────────────
WORK_DIR=$(mktemp -d "/tmp/droste-bar.XXXXXX")
...
OUTPUT_TARBALL="${PROJECT_DIR}/output/droste-bar.tar.xz"
...
if $DO_IMPORT; then
    echo "Importing into $CONTAINER_CMD as droste-bar..."
    # podman import chokes on .tar.xz — decompress for import, then clean up
    IMPORT_TARBALL="${WORK_DIR}/droste-bar-import.tar"
    xz -dc "$OUTPUT_TARBALL" > "$IMPORT_TARBALL"
    $CONTAINER_CMD import "$IMPORT_TARBALL" droste-bar
    rm -f "$IMPORT_TARBALL"
    echo ""
    echo "Image imported."
    $CONTAINER_CMD image inspect droste-bar --format '{{.Size}}' | \
        awk '{printf "Image size: %.0f MB\n", $1/1024/1024}'
fi

# ── Summary ─────────────────────────────────────────────────────────
echo ""
echo "droste-bar built successfully."
echo "  Source:   $SEED_TARBALL"
echo "  Removed:  $(echo $EXCLUDE_PKGS | wc -w) packages"
echo "  Tarball:  $OUTPUT_TARBALL ($TARBALL_SIZE)"
if $DO_IMPORT; then
    echo "  Image:    droste-bar ($CONTAINER_CMD)"
fi
